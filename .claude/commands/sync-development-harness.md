...
4. Gather workspace ground truth:
...
7. Update `.harness/phase-graph.json` unit statuses only with evidence:
...
8. Update `.harness/state.json`:
   - Set `drift.last_sync` to current ISO timestamp
   - Record divergences in `drift.divergences`
   - Update `execution` pointers if needed
9. If phase documents need updates, apply conservatively — add info, never remove intent.
10. Update `.harness/checkpoint.md` with post-sync state.
11. Output sync report:
